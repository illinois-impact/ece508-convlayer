...
static int generate_data(float *x, const shape xdims) {
  // input dimension size
  std::cout << "input dimensions = " << xdims.num << " x " << xdims.depth << " x " << xdims.height << " x "
            << xdims.width << "\n";
  ...
  for (const auto i : range(0, xdims.num)) {
    for (const auto c : range(0, xdims.depth)) {
      for (const auto h : range(0, xdims.height)) {
        for (const auto w : range(0, xdims.width)) {
          // LESLIE: double check the offset computation
          const auto xoffset = i * xdims.depth * xdims.height * xdims.width + c * xdims.height * xdims.width + 
                               h * xdims.width + w;
...
static void generate_convfilters(float *conv, const shape convdim) {
  ...
  std::cout << "filter dimensions = " << convdim.num << " x " << convdim.depth << " x " <<
               convdim.height << " x " << convdim.width << "\n";

  // Set convolution filter values to 1
  std::memset(conv, 1, convdim.num * convdim.depth * convdim.height * convdim.width);
}
...
static void average_pool(const float *X, const shape xdims, const int pool_size, 
          ...
          for (const auto p : range(0, wdims.height)) {     // filter height
            for (const auto q : range(0, wdims.width)) {    // filter width
              for (const auto c : range(0, xdims.depth)) {   // sum over all input feature maps
                const auto yoffset = ((i * ydims.depth + h) * ydims.height + w) * ydims.width + m;
                const auto xoffset = i * xdims.depth * xdims.height * xdims.width + (h + p) *
                                     xdims.height * xdims.width + (w + q) * xdims.width + c;
                const auto woffset = p * wdims.depth * wdims.height * wdims.width + 
                                     q * wdims.height * wdims.width + c * wdims.width + m;
...
static void conv_backward_ygrad(const float *Y_orig, const float *Y, const shape ydims, 
                                float *dE_dY, const shape dedydims) {
  for (const auto i : range(0, ydims.num)) {
    for (const auto m : range(0, ydims.depth)) {        // for each output feature map
      for (const auto h : range(0, ydims.height)) {     // for each output element
        for (const auto w : range(0, ydims.width)) {
...
static void conv_backward_xgrad(const float *X, const shape xdims, const float *W, const shape wdims, 
                                const float *Y, const shape ydims, const float *dE_dY, float *dE_dX) {
...
void forward_operation(float *x, float *conv1, float *conv2, float *fc1, float *fc2, int *out) {
  // conv layer
  const shape adims = {xdims.num, conv1dims.depth, (xdims.height - conv1dims.height + 1), (xdims.width -
      conv1dims.width + 1)};
  ...
  // conv layer
  const shape cdims = {bdims.num, conv2dims.depth, (bdims.height - conv2dims.height + 1), (bdims.width -
      conv2dims.width + 1)};
...
void backward_operation(float *x, float *conv1, float *conv2, float *fc1, float *fc2, float *y, const float *y_orig) {
  // conv layer
  const shape dydims = {xdims.num, conv1dims.depth, (xdims.height - conv1dims.height + 1), (xdims.width -
      conv1dims.width + 1)};
