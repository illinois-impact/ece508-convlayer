...
struct shape {
  ...
  shape(size_t height, size_t width) : num(1), depth(1),
...
static enable_if_t<std::is_integral<SzTy>::value, T> *
allocate(const SzTy len) {
  T *res = new T[len];
  return res;
}

template <typename T, typename ShapeT>
static enable_if_t<std::is_same<ShapeT, shape>::value, T*>
allocate(const ShapeT & shp) {
